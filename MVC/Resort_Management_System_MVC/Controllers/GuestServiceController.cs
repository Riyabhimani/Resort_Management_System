...
//            try
//            {
//                // ✅ Check Guest has confirmed reservation
//                var resResponse = await client.GetAsync("GuestServices/dropdown/reservations");
//                if (resResponse.IsSuccessStatusCode)
//                {
//                    var reservations = JsonConvert.DeserializeObject<List<ReservationModel>>(
...
//                    if (model.ReservationId == 0)
//                    {
//                        ModelState.AddModelError("ReservationId", "Selected guest does not have a confirmed reservation.");
//                    }
//                }
//                else
//                {
//                    ModelState.AddModelError("ReservationId", "Unable to fetch reservations from API.");
//                }
...
//                // ✅ API call
//                var content = new StringContent(JsonConvert.SerializeObject(model), Encoding.UTF8, "application/json");
//                HttpResponseMessage response = model.GuestServiceId == 0
//                    ? await client.PostAsync("GuestServices", content)
//                    : await client.PutAsync($"GuestServices/{model.GuestServiceId}", content);

//                if (!response.IsSuccessStatusCode)
//                {
//                    // API validation error response read
//                    var error = await response.Content.ReadAsStringAsync();
//                    TempData["ErrorMessage"] = $"API failed: {response.StatusCode} → {error}";
//                    await LoadGuests(model.GuestId);
//                    await LoadServices(model.ServiceId);
//                    return View(model);
//                }
...
//                // ✅ Success
//                TempData["SuccessMessage"] = model.GuestServiceId == 0 ? "Guest Service added successfully." : "Guest Service updated successfully.";
//                return RedirectToAction("GuestServiceList");
//            }
//            catch (Exception ex)
//            {
...
namespace Resort_Management_System_MVC.Controllers
{
    public class GuestServiceController : Controller
    {
        ...
        // ✅ List all Guest Services
        public async Task<IActionResult> GuestServiceList()
        {
            var response = await client.GetAsync("GuestServices");
            if (!response.IsSuccessStatusCode)
            {
                TempData["ErrorMessage"] = "Unable to fetch guest services.";
                return View(new List<GuestServiceModel>());
            }
        ...
        public async Task<IActionResult> GuestServiceDelete(int id)
        {
            try
            {
                var response = await client.DeleteAsync($"GuestServices/{id}");
                if (!response.IsSuccessStatusCode)
                {
                    TempData["ErrorMessage"] = "Failed to delete Guest Service.";
                }
                else
                {
                    TempData["SuccessMessage"] = "Guest Service deleted successfully.";
                }
            }
            catch (Exception ex)
            {
                TempData["ErrorMessage"] = "Error while deleting: " + ex.Message;
            }
        ...
        public async Task<IActionResult> GuestServiceAddEdit(GuestServiceModel model)
        {
            try
            {
                // Get Confirmed Reservation for selected Guest
                var resResponse = await client.GetAsync("GuestServices/dropdown/reservations");
                if (resResponse.IsSuccessStatusCode)
                {
                    var reservations = JsonConvert.DeserializeObject<List<ReservationModel>>(
                ...
                TempData["SuccessMessage"] = model.GuestServiceId == 0
                    ? "Guest Service added successfully."
                    : "Guest Service updated successfully.";
        ...
        // ✅ AJAX - Get Guests by Reservation
        public async Task<JsonResult> GetGuestsByReservation(int reservationId)
        {
            var response = await client.GetStringAsync($"GuestServices/dropdown/guest/{reservationId}");
            var guests = JsonConvert.DeserializeObject<List<GuestModel>>(response);
        ...
        public async Task<IActionResult> Top10GuestServices()
        {
            ...
            var json = await response.Content.ReadAsStringAsync();
            var list = JsonConvert.DeserializeObject<List<GuestServiceModel>>(json);
            return View("GuestServiceList", list); // reuse same view
        }
